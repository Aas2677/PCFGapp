...
class ProbabilisticCYKParser:
    ...
    """
    Sets up the neccessary data structures for parsing with probabilistic grammars 
    contains all the neccessary methods to find the most probable parse of a string, total probability, count of parses and N-best parses
    """
    ...
    def get_bytes_output(self,parses) -> list:
    ...
    def best_parse_all(self,input_string, is_sentence = True):
        ...
        """
        Finds the best parse of an input string or setnence 
        Returns either a grammar error if the parse is failed, or a probabilistic node representing the best parse if successful 
        """
        ...
        # Parse through the string for partition length > 1 
    ...
    def next_tree(self,node , n : int) -> None:
        ...
        Manipulates lists in the container attribute so does not return anything at all 
        ...
        starter_root,stop_root = node.span
        ...
        # If we are askng for the second best parse, we need to construct our consideration table from scratch, otherwise we can skip this because the consider set should still have trees in it.
        if n == 2:
            if self._grammar.expansion_lhs_lookup[root_token]:
                for rule in self._grammar.expansion_lhs_lookup_logs[root_token]:
                    left,right = rule._right[0]._token,rule._right[1]._token
                    for (left_start,left_stop),(right_start,right_stop) in self.partitions_2(starter_root,stop_root):
                        if self._n_best_table[left_start,left_stop,left] and self._n_best_table[right_start,right_stop,right]:
                            ...
                            self._n_best_consider[starter_root,stop_root,root_token].append(ProbabilisticNode(rule, self._n_best_table[left_start,left_stop,left],self._n_best_table[right_start,right_stop,right],this_weight,node.span,rank=2))

                candidates = sorted(self._n_best_consider[starter_root,stop_root,node.rule._left._token], key = lambda x : x.cumulative_prob)
                candidates.pop(0)
                self._n_best_consider[starter_root,stop_root,node.rule._left._token] = candidates
                                

        

        # If the next best tree for the lefthand subtree has not been computed, we need to compute it recursivley.
        ...
        # If the next best tree for the lefthand subtree exists, we can add it to our consider set
        if node.right_child.rank == 1 and len(self._n_best_dict[start_left,stop_left,node.left_child.rule._left._token]) >= node.left_child.rank + 1:
            ...
            self._n_best_consider[starter_root,stop_root,node.rule._left._token].append((ProbabilisticNode(node.rule,new_left_tree,node.right_child,probability,node.span,rank=n)))
        ...
        # If the next best tree for the righthand subtree exists, we can add it to the consider set.
        if len(self._n_best_dict[start_right,stop_right,node.right_child.rule._left._token]) >= node.right_child.rank + 1:
            ...
            self._n_best_consider[starter_root,stop_root,node.rule._left._token].append((ProbabilisticNode(node.rule,node.left_child,new_right_tree,probability,node.span,rank=n)))
        ...
        if candidates:
            ...
            candidates.pop(0)
            self._n_best_dict[starter_root,stop_root,node.rule._left._token].append(next_best_tree)
            self._n_best_consider[starter_root,stop_root,node.rule._left._token] = candidates
        else:
    ...
    def total_probability(self,input_string, is_sentence = True):
        ...
        """
        finds the total probability of the input string being generated by the grammar. Throws error if not accepted.
        If accepted, returns a probabilisticnode whose cumulative probability is the total probability of a parse
...
class ProbabilisticNode:
    ...
    Represents probabilistic or non-probabilistic parse trees
    contains all the methods needed to unfold into a JSON structure 
    ...
    def get_full_tree(self,table=False,add_messages=True) -> dict:
